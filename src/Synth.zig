...
pub const Slot = struct {
    ...
    pub fn init(params: Params, env_params: Envelope.Params) Slot {
        return .{
            .t = 0.0,
            .v = 0.0,
            .params = params,
    ...
    pub fn update(slot: *Slot, freq: f32, phase: f32) void {
        ...
        if (effective_freq > 0.0) {
            slot.v = slot.params.tl * slot.env.v * @sin(effective_freq * slot.t * std.math.tau + phase + slot.params.fb * slot.v);
            slot.t = @mod(slot.t + sample_time, 1 / effective_freq);
        } else {
            slot.v = 0.0;
        }
    }
