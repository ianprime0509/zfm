...
pub fn sourceLocation(c: *const Compiler, index: SourceIndex) SourceLocation {
    var loc: SourceLocation = .{ .line = 1, .column = 1 };
    var pos: SourceIndex = .start;
    while (pos != index) : (pos = pos.next()) {
...
pub fn toModule(c: *Compiler) Allocator.Error!Module {
    ...
    for (c.parts.values(), parts) |part, *mod_part| {
        ...
        mod_part.* = .{
            .start = start,
            .global_loop = start.offset(.between(.zero, global_loop)),
        };
        commands.appendAssumeCapacity(.{
            .tag = .end,
            .data = .{ .none = {} },
...
fn compileDirective(c: *Compiler) !void {
    ...
    const directive = std.meta.stringToEnum(enum {
        ...
        try c.reportPos(.invalid_directive, name_pos);
        c.skipLineAndContinuation();
        return;
    };
    try c.expectSpace();
...
fn compilePatch(c: *Compiler) !void {
    ...
    for (&slot_params, &slot_env_params) |*params, *env_params| {
...
fn compileCommand(c: *Compiler, part: *Part) !void {
    switch (c.peekByte()) {
        ...
        '.' => {
            ...
            const rest = previousRestIndex(tags) orelse return c.reportPos(.last_command_not_note, pos);
            datas[rest].ticks = datas[rest].ticks.dot() catch return c.reportPos(.cannot_dot, pos);
        },
        'l' => {
            c.skipByte();
        ...
        '&' => {
            ...
            if (tags.len < 1 or tags[tags.len - 1] != .key_off) {
                return c.reportPos(.last_command_not_note, pos);
            }
            part.commands.len -= 1;
        },
        'o' => {
        ...
        '@' => {
            ...
            const name_str = c.strings.find(name) orelse return c.report(.undefined_patch);
            const index = c.patches.get(name_str) orelse return c.report(.undefined_patch);
        ...
        '[' => {
            const pos = c.pos;
            c.skipByte();
            part.loops.push(.{ .start = part.nextCommandIndex(), .pos = pos }) catch
                return c.reportPos(.loop_too_deep, pos);
        },
        ']' => {
            const pos = c.pos;
            c.skipByte();
            ...
            if (part.loops.pop()) |loop| {
                ...
                return c.reportPos(.not_in_loop, pos);
            }
        },
        '{' => {
            c.skipByte();
...
fn addSetLfoTargetCommand(c: *Compiler, part: *Part, index: Lfo.Index, target: Lfo.Target) !void {
    ...
    try c.extra.encode(c.gpa, target);
    try part.addCommand(c.gpa, .set_lfo_target, .{ .lfo_data = .{
        .index = index,
        .data = data,
    } });
}
    ...
    while (true) {
        ...
        switch (c.peekByte()) {
            eof, ' ', '\t', '\r', '\n' => break,
                ...
                try c.report(.invalid_part_name);
                c.skipByte();
            },
        }
    }
