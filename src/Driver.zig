...
pub fn init(synth: *Synth, mod: *const Module, parts: []Part) Driver {
    ...
    for (parts, mod.parts) |*part, start| part.* = .init(start);
    return .{
        .synth = synth,
        .mod = mod,
        .parts = parts,
...
fn execute(
    ...
    switch (driver.mod.tag(command)) {
        ...
        .key_on => {
            part.synth_params.voice.freq = driver.mod.data(command).freq;
        ...
        .set_lfo_target => {
            ...
            lfo.target, _ = driver.mod.extra.decode(Lfo.Target, lfo_data.data);
            return command.next();
        },
        .set_lfo_size => {
            const lfo_data = driver.mod.data(command).lfo_data;
            const lfo = part.lfoPtr(lfo_data.index);
            lfo.size, _ = driver.mod.extra.decode(Lfo.Size, lfo_data.data);
            return command.next();
        },
        .set_lfo_wave => {
            const lfo_data = driver.mod.data(command).lfo_data;
            const lfo = part.lfoPtr(lfo_data.index);
            lfo.wave, _ = driver.mod.extra.decode(Lfo.Wave, lfo_data.data);
            return command.next();
        },
        .loop => {
...
pub const Part = struct {
    ...
    fn init(mod_part: Module.Part) Part {
        ...
        part.lfos.getPtr(.porta).time_unit = .ticks;
        return part;
    }
    ...
    pub const LfoState = struct {
        ...
        fn process(lfo: *LfoState, sp: *SynthParams, elapsed_samples: Samples, tempo: Tempo) void {
            ...
            const v = lfo.sample(elapsed_samples, tempo);
            switch (lfo.target) {
                .freq => sp.voice.freq += v,
            }
        }

        fn sample(lfo: *LfoState, elapsed_samples: Samples, tempo: Tempo) f32 {
            const elapsed_time = switch (lfo.time_unit) {
            ...
            const base = base: switch (lfo.wave) {
                .constant => 1.0,
                .sine => |params| {
                ...
                .exp => |params| {
                    const v = @exp2(params.mul * lfo.t);
                    lfo.t += elapsed_time;
                    break :base v;
                },
            };
            return lfo.size.scale * base + lfo.size.offset;
        }
    };
